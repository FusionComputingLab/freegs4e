...
def test_set_threads():
    ...
    num_threads = parallel_funcs.get_num_threads()
    parallel_funcs.set_num_threads(num_threads + 2)

    assert parallel_funcs.get_num_threads() == (num_threads + 2)

    parallel_funcs.set_num_threads(num_threads)
...
def test_take(make_test_array):
    ...
    orig_num_threads = parallel_funcs.get_num_threads()
    parallel_funcs.set_num_threads(1)
    assert np.all(
        reference == parallel_funcs.threaded_take(make_test_array, idcs)
    ), "Thin wrapper fail (failed at 1 thread)"

    parallel_funcs.set_num_threads(2)
    assert np.all(
        reference == parallel_funcs.threaded_take(make_test_array, idcs)
    ), "Failed on 2 threads"

    parallel_funcs.set_num_threads(61)
    assert np.all(
        reference == parallel_funcs.threaded_take(make_test_array, idcs)
    ), "Failed n_threads>idcs.shape[0]"

    parallel_funcs.set_num_threads(orig_num_threads)
...
def test_take_axis(make_test_array):
    ...
    assert np.all(
        reference
        == parallel_funcs.threaded_take(make_test_array, idcs, axis=1)
    ), "Thin wrapper fail (failed at 1 thread)"
    ...
    parallel_funcs.set_num_threads(21)
    assert np.all(
        reference
        == parallel_funcs.threaded_take(make_test_array, idcs, axis=1)
    ), "Failed n_threads>idcs.shape[0]"
...
def test_clip(make_test_array):
    ...
    parallel_funcs.set_num_threads(1)
    assert np.all(
        reference == parallel_funcs.threaded_clip(make_test_array, amin, amax)
    ), "Thin wrapper fail (failed at 1 thread)"
    ...
    parallel_funcs.set_num_threads(101)
    assert np.all(
        reference == parallel_funcs.threaded_clip(make_test_array, amin, amax)
    ), "Failed n_threads>idcs.shape[0]"
...
def test_elliptics(make_test_array):
    ...
    orig_num_threads = parallel_funcs.get_num_threads()
    parallel_funcs.set_num_threads(1)
    rese, resk = parallel_funcs.threaded_elliptics_ek(make_test_array)
    assert np.all(
        (refe == rese) & (refk == resk)
    ), "Thin wrapper fail (failed at 1 thread)"
